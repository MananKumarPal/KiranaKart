...
"""
#cur.execute("drop database KiranaKart")
try:
    cur.execute("create database KiranaKart")
    cur.execute("use KiranaKart")
...
def insert():
    ...
    if(qty!=''):
        if (int(qty)>0 and int(qty)<int(sqty[0])):
            ...
            db.commit()
            messagebox.showinfo("KiranaKart ","Succesful!")
        elif(int(qty)>int(sqty[0])):
            messagebox.showinfo("KiranaKart ","Sorry. We have inadequate stocks")
        else:
            messagebox.showinfo("KiranaKart ","Please set the quantity to atleast 1")
    else:
        messagebox.showinfo("KiranaKart ","Quantity is a mandatory field and cannot be left empty")
...
def update():
    ...
    db.commit()
    messagebox.showinfo("KiranaKart","Updation of records will be highlited when you again visit this section")
...
def delete():
    ...
    db.commit()
    messagebox.showinfo("KiranaKart","deletion of records will be highlited when you again visit this section")
...
def back():
    ...
    Label(root,image=img2).place(x=60,y=0)
    Label(root,text="©2022",font="arial 7 ").place(x=60,y=113)

    Label(root,text="You need it , We got it ",font="arial 14 bold").place(x=150,y=140)
    Label(root,text="How do you wish to continue?",font="arial 14 bold").place(x=120,y=180)
    ...
    Label(root,text="Admin",font="arial 14 bold").place(x=110,y=350)

    Button(root,text="CUSTOMER",command=customer,image= P2).place(x=300,y=230)
    Label(root,text="Customer",font="arial 14 bold").place(x=310,y=350)
...
def exitt():
    ...
    if (name=="" or name==" "):
        messagebox.showinfo("KiranaKart","Mandatory (*) Fields cannot be empty")
    else:
...
def Goback():
    ...
    try:
        ...
        Label(root1,image=img2).place(x=60,y=0)
        Label(root1,text="©2022",font="arial 7 ").place(x=60,y=113)

        Label(root1,text="WHAT DO YOU WANT TO DO?",font="arial 14 bold").place(x=110,y=170)

        Label(root1,text="First lets Add items to cart ?",font="arial 8 bold").place(x=160,y=200)
...
def insertscr():
    ...
    Label(root2,text="©2022",font="arial 7 ").place(x=60,y=113)
    ...
    Label(root3,image=imag2).place(x=60,y=0)
    Label(root3,text="©2022",font="arial 7 ").place(x=60,y=113)

    Label(root3,text="Product code",font="arial 11 ").place(x=50,y=150)
    Label(root3,text="Product Name",font="arial 11 ").place(x=200,y=150)
    ...
    for x in rec:
        ...
        Label(root3,text=x[1],width=12,relief=GROOVE).place(x=210,y=i)
...
def deletescr():
    ...
    if (reco==[]):
        messagebox.showinfo("KiranaKart","Cart empty! Cannot proceed.")
    else:    
        root1.destroy()
        ...
        Label(root2,image=img2).place(x=60,y=0)
        Label(root2,text="©2022",font="arial 7 ").place(x=60,y=113)
...
def Cpurchase():
    ...
    Button(root2,text="Card",command=FSCR, image= P1).place(x=70,y=220)
    Label(root2,text="Credit/Debit Card",font="arial 14 bold").place(x=40,y=370)

    Button(root2,text="Cash",command=FSCR,image= P2).place(x=320,y=220)
...
company (KiranaKart here) distributes the default passwords and username to the amdins and it can never be changed..
...
def aupdate():
    ...
    cur.execute(sql1,val)
    db.commit()
    messagebox.showinfo("KiranaKart","Stocks updated successfully")
...
def goback():
    ...
    Button(root2,text="Update Stock",command=SUpdate,image=P1).place(x=60,y=150)
    Button(root2,text="Check Stock",command=Sshow,image=P2).place(x=315,y=150)
    Button(root2,text="Logout",command=logout,image=P3).place(x=420,y=400)
    Button(root2,text="Sales Check",command=Salecheck,image=P4).place(x=190,y=300)

    Label(root2,text="Manage Stock",font="arial 14 bold").place(x=50,y=266)
    ...
    Label(root2,text="Total Sales Check",font="arial 14 bold").place(x=160,y=430)
...
def login():
    ...
    if i in rec:
        ...
        Label(root2,text="Manage Stock",font="arial 14 bold").place(x=50,y=266)
        Label(root2,text="Check Stock",font="arial 14 bold").place(x=310,y=266)
        Label(root2,text="Logout",font="arial 14 bold").place(x=415,y=461)
        Label(root2,text="Total Sales Check",font="arial 14 bold").place(x=160,y=430)

    else:
        messagebox.showinfo("KiranaKart","Invalid Credentials! Please try again")
